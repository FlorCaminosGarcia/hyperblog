 ...
 * # Run this script
 * npm install @modelcontextprotocol/sdk @azure-rest/ai-inference @azure/core-auth
...
{{/parameters.presence_penalty}}
            ...
            if (response.body.choices[0].message.tool_calls) {
                for (const tool of response.body.choices[0].message.tool_calls) {
                    hasToolCall = true;
...
{{#isImage}}
                {
                    type: "image_url",
                    image_url: {
                        url: encodeImage({{{localPathWithQuote}}}, "{{{mimeType}}}")
                    }
                },
{{/isImage}}
{{/content}}
